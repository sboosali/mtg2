...
parseCardObjectJSON = JSON.withObject "CardObject" pO
  ...
  pO o = do
    ...
    _multiverseId           <- o .:? "multiverseId"           .!= 0

    _name                   <- pRequired "name" id o
...
pRequired fieldName toValue o =
  ...
  (fmap toValue (o .: fieldName))

--------------------------------------------------

pOptional :: Text -> (Text -> a) -> a -> JSON.Object -> JSON.Parser a
pOptional fieldName toValue defaultValue o =

  ((fmap.fmap) toValue (o .:? fieldName)) .!= defaultValue
